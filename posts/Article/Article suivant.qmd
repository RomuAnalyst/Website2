...
ui <- fluidPage(
  selectInput("variable", "Sélectionnez une variable :", choices = metro$name),
  textOutput("selected_variable")
)

server <- function(input, output) {
  output$selected_variable <- renderText({
    paste("Vous avez sélectionné :", input$variable)
  })
}
...
```{r}
metro_modifstr <- head(metro) %>% rename(Station = name, Lignes = lines, Lieux = caption, connect_autre = connect_other, Nombre_de_passagers = passengers, localisation = location)
